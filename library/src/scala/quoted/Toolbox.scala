...
trait Toolbox {
  ...
  protected def runImpl[T](code: QuoteContext => Expr[T]): T // For Scala2 compat in ToolboxImpl

  def show[T](code: Staged[T]): String = runImpl(ctx => { implicit val c: QuoteContext = ctx; code(ctx).show.toExpr })
}
