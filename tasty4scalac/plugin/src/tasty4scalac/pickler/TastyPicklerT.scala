...
class TastyPicklerT[A <: AST](val translator: ASTTranslator[A]) {
  import translator.names

  private val sections = new mutable.ArrayBuffer[(NameRef, TastyBuffer)]

  val nameBuffer: NameBufferT[A] = new NameBufferT[A](translator)

  def newSection(name: String, buf: TastyBuffer): Unit =
    sections += ((nameBuffer.nameIndex(names.toTermName(name)), buf))
