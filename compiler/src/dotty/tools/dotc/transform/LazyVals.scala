...
class LazyVals extends MiniPhase with IdentityDenotTransformer {
  ...
  def transformMemberDefScala2Compat(x: ValOrDefDef)(implicit ctx: Context): Thicket = {
    ...
    val flagSymbol = ctx.newSymbol(owner, flagName,  containerFlags | Private, defn.BooleanType).enteredAfter(this)
    ...
    val initSymbol = ctx.newSymbol(owner, initName,
      initFlags | Private, MethodType(Nil, tpe), coord = x.symbol.coord
    ).enteredAfter(this)
    val rhs = x.rhs.changeOwnerAfter(x.symbol, initSymbol, this)
