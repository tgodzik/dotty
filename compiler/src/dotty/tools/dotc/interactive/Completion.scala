...
object Completion {
  ...
  private class CompletionBuffer(val mode: Mode, val prefix: String, pos: SourcePosition) {
    ...
    private def add(sym: Symbol, nameInScope: Name)(implicit ctx: Context) =
      if (sym.exists && completionsFilter(NoType, nameInScope) && !completions.lookup(nameInScope).exists && include(sym, nameInScope)) {
    ...
    private def accessibleMembers(site: Type)(implicit ctx: Context): Seq[Symbol] = site match {
      case site: NamedType if site.symbol.is(Package) =>
        site.decls.toList.filter(sym => sym.isAccessibleFrom(site, superAccess = false)) // Don't look inside package members -- it's too expensive.
      case _ =>
