...
object Complex {
  implicit def isLiftable[T: Type: Liftable]: Liftable[Complex[T]] = new Liftable[Complex[T]] {
    def toExpr(comp: Complex[T])(implicit st: StagingContext): Expr[Complex[T]] = '(Complex(~comp.re.toExpr, ~comp.im.toExpr))
  }
}
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def code: Staged[Array[Int] => Int] = '{(arr: Array[Int]) => ~dotIntOptExpr(implicitly)(Vec(5, i => Dyn('(arr(~i.toExpr)))), staticVec).expr }
