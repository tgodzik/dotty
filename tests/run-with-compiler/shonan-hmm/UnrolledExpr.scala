...
object UnrolledExpr {
  ...
  def block[T](stats: Iterable[Expr[_]], expr: Expr[T]): Staged[T] = {
...
class UnrolledExpr[T: Liftable, It <: Iterable[T]](xs: It) {
  ...
  def foreach[U](f: T => Expr[U]): Staged[Unit] = block(xs.map(f), '())

  def withFilter(f: T => Boolean): UnrolledExpr[T, Iterable[T]] = new UnrolledExpr(xs.filter(f))

  def foldLeft[U](acc: Expr[U])(f: (Expr[U], T) => Expr[U]): Staged[U] =
