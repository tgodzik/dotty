...
object Test {
  def main(args: Array[String]): Unit = {
    ...
    def liftedList: Staged[List[Int]] = list.toExpr

    tb.show(liftedList.foldLeft[Int](0.toExpr)('{ (acc: Int, x: Int) => acc + x }))
    tb.show(liftedList.foreach('{ (x: Int) => println(x) }))

    tb.show(list.unrolledFoldLeft[Int](0.toExpr)('{ (acc: Int, x: Int) => acc + x }))
...
package liftable {
  ...
  object Exprs {
    implicit class LiftExprOps[T](x: T) extends AnyVal {
      def toExpr(implicit liftable: Liftable[T], ctx: QuoteContext): Expr[T] =
        liftable.toExpr(x)
    }
  }
  ...
  object Units {
    implicit def UnitIsLiftable: Liftable[Unit] = new Liftable[Unit] {
      override def toExpr(x: Unit)(implicit qCtx: QuoteContext): Expr[Unit] = '()
    }
  }

  object Lets {
    def letVal[T, U](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Type[T]): Staged[U] =
      '{ val letVal: ~t = ~expr; ~body('(letVal)) }
    def letLazyVal[T, U](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Type[T]): Staged[U] =
      '{ lazy val letLazyVal: ~t = ~expr; ~body('(letLazyVal)) }
    def letDef[T, U](expr: Expr[T])(body: Expr[T] => Expr[U])(implicit t: Type[T]): Staged[U] =
  ...
  object Tuples {
    ...
    implicit def Tuple1IsLiftable[T1: Liftable](implicit t1: Type[T1]): Liftable[Tuple1[T1]] = new Liftable[Tuple1[T1]] {
      def toExpr(x: Tuple1[T1])(implicit qCtx: QuoteContext): Expr[Tuple1[T1]] =
    ...
    implicit def Tuple2IsLiftable[T1: Liftable, T2: Liftable](implicit t1: Type[T1], t2: Type[T2]): Liftable[(T1, T2)] = new Liftable[(T1, T2)] {
      def toExpr(x: (T1, T2))(implicit qCtx: QuoteContext): Expr[(T1, T2)] =
    ...
    implicit def Tuple3IsLiftable[T1: Liftable, T2: Liftable, T3: Liftable](implicit t1: Type[T1], t2: Type[T2], t3: Type[T3]): Liftable[(T1, T2, T3)] = new Liftable[(T1, T2, T3)] {
      def toExpr(x: (T1, T2, T3))(implicit qCtx: QuoteContext): Expr[(T1, T2, T3)] =
    ...
    implicit def Tuple4IsLiftable[T1: Liftable, T2: Liftable, T3: Liftable, T4: Liftable](implicit t1: Type[T1], t2: Type[T2], t3: Type[T3], t4: Type[T4]): Liftable[(T1, T2, T3, T4)] = new Liftable[(T1, T2, T3, T4)] {
      def toExpr(x: (T1, T2, T3, T4))(implicit qCtx: QuoteContext): Expr[(T1, T2, T3, T4)] =
  ...
  object Lists {
    implicit def ListIsLiftable[T: Liftable](implicit t: Type[T]): Liftable[List[T]] = new Liftable[List[T]] {
      def toExpr(x: List[T])(implicit qCtx: QuoteContext): Expr[List[T]] = x match {
    ...
    object Arrays {
      implicit def ArrayIsLiftable[T: Liftable](implicit t: Type[T], ct: Expr[ClassTag[T]]): Liftable[Array[T]] = new Liftable[Array[T]] {
        def toExpr(arr: Array[T])(implicit qCtx: QuoteContext): Expr[Array[T]] = '{ new Array[~t](~arr.length.toExpr)(~ct) }
      }
    }
