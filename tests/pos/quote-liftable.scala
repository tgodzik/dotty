...
object Test {
  ...
  implicit def IntIsLiftable: Liftable[Int] = new {
    def toExpr(n: Int)(implicit ctx: QuoteContext) = n match {
  ...
  implicit def BooleanIsLiftable: Liftable[Boolean] = new {
    def toExpr(b: Boolean)(implicit ctx: QuoteContext) =
      if (b) '(true) else '(false)
  }

  implicit def ListIsLiftable[T: Liftable: Type]: Liftable[List[T]] = new {
    def toExpr(xs: List[T])(implicit ctx: QuoteContext) = xs match {
      case x :: xs1 => '{ ~implicitly[Liftable[T]].toExpr(x) :: ~toExpr(xs1) }
      case Nil => '(Nil: List[T])
    }
  }

  implicit val dummy: QuoteContext = ???
