...
object Optional {
  ...
  /*private*/ def getOrElseImpl[T >: Null](opt: Expr[Optional[T]], alt: Expr[T]): Staged[T] = '{
    if ((~opt).isEmpty) ~alt else (~opt).value
  }

  // FIXME fix issue #5097 and enable private
  /*private*/ def mapImpl[A >: Null, B >: Null : Type](opt: Expr[Optional[A]], f: Expr[A => B]): Staged[Optional[B]] = '{
