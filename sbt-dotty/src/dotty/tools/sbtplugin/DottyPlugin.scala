  ...
  override val globalSettings: Seq[Def.Setting[_]] = Seq(
    onLoad in Global := onLoad.in(Global).value.andThen { state =>
      ...
      sbtFullVersion(sbtV) match {
        case Some((1, sbtMinor, sbtPatch)) if sbtMinor > 1 || (sbtMinor == 2  && sbtPatch >= 3) =>
        case _ =>
          sys.error(s"The sbt-dotty plugin cannot work with this version of sbt ($sbtV), sbt >= 1.2.3 is required.")
      }
      state
    }
  )
